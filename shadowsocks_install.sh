...
cd libsodium-1.0.16 && ./configure --prefix=/usr/local/libsodium && make && make install
...
cd mbedtls-2.6.0 && make SHARED=1 CFLAGS=-fPIC && make DESTDIR=/usr/local/mbedtls install

echo "/usr/local/mbedtls/lib" >> /etc/ld.so.conf && ldconfig

git clone https://github.com/shadowsocks/shadowsocks-libev.git && cd shadowsocks-libev/ && git submodule update --init --recursive && ./autogen.sh && ./configure --prefix=/usr/local/shadowsocks --with-mbedtls=/usr/local/mbedtls/ --with-sodium=/usr/local/libsodium/ && make && make install
